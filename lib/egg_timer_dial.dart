...
import 'package:egg_timer/egg_timer_dial_knob.dart';
...
class _EggTimerDialState extends State<EggTimerDial> with TickerProviderStateMixin {
  ...
  double prevRotationPrecent = 0.0;
  ...
  Widget build(BuildContext context) {
    ...
    if( widget.currentTime.inSeconds == 0 && prevEggTimerState != EggTimerState.ready) {
      resettingAnimation = Tween(begin: prevRotationPrecent, end: 0.0 )
      ...
      resetToZeroController.duration = Duration(
        milliseconds: ((prevRotationPrecent / RESET_SPEED_PERCENT_PER_SECOND) * 1000).round()
      );
      resetToZeroController.forward(from: 0.0);
    }
    prevEggTimerState = widget.eggTimerState;
    prevRotationPrecent = _rotationPercent();

    

    return DialTurnGustureDetector(
                    ...
                    Container(
                      ...
                      child: CustomPaint(
                        painter: TickPainter(
                          ...
                          ticksPerSection: widget.ticksPerSecond,
                        ),
                      ),
                    ),
                    Padding(
...
class DialTurnGustureDetector extends StatefulWidget {
  final currentTime;
  final maxTime;
  final child;
  final Function(Duration) onTimeSelected;
  ...
  @override
  _DialTurnGustureDetectorState createState() =>
      _DialTurnGustureDetectorState();
}

class _DialTurnGustureDetectorState extends State<DialTurnGustureDetector> {
  ...
  _onRadialDragUpdate(PolarCoord coord) {
    if (startDragCoord != null) {
      ...
      final anglePerent = angleDiff / (2 * pi);
      final timeDiffInSeconds = (anglePerent * widget.maxTime.inSeconds).round();
